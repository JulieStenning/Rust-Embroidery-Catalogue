...
mod tests {
    ...
    // ─── Helper: seed a database with the initial schema tables ──────────────
    async fn seed_schema_tables(pool: &SqlitePool) {
        // Apply the DDL from migration 1 (all use IF NOT EXISTS, so safe to re-run)
        sqlx::query(include_str!("../../migrations/20260503000000_initial.up.sql"))
            .execute(pool)
            .await
    ...
    async fn locked_migration_true_for_execute_with_busy_inner() {
        ...
        // ── Error for MigrateError::Execute ───────────────────────────────
        ...
        // ── Error for MigrateError::ExecuteMigration ──────────────────────
        ...
        let migrate_err_2 =
            MigrateError::ExecuteMigration(sqlite_err_execmig, 20260730000000_i64);
        assert!(
    ...
    // ─── run_migrations — happy path on a fresh DB ───────────────────────────
    ...
    // ─── run_migrations — idempotent (run twice) ────────────────────────────
    ...
    async fn run_migrations_is_idempotent() {
        ...
        assert!(r2.is_ok(), "second migration run should succeed (idempotent)");
    ...
    // ─── run_migrations — seed DB scenario (pre-created schema, no tracking) ─
    ...
    async fn run_migrations_catches_index_already_exists_on_re_migrate() {
        ...
        // Drop the _sqlx_migrations tracking table — this simulates a seed DB
        ...
        //   Migration 3 (plain CREATE INDEX):  fails → guard catches "already exists".
    ...
    async fn run_migrations_handles_seeded_schema_with_only_idempotent_migrations() {
        ...
        // Apply migrations 1 + 2 (all IF NOT EXISTS — safe to pre-apply).
        ...
        // - Migration 3:   CREATE INDEX hits "already exists" → guard catches it.
