...
async fn get_design_filepath(pool: &SqlitePool, design_id: i64) -> Result<String, String> {
	let filepath = sqlx::query_scalar::<_, String>(
		"SELECT filepath FROM designs WHERE id = ? LIMIT 1",
	)
	.bind(design_id)
	.fetch_optional(pool)
	.await
	...
	match filepath {
		...
		Some(_) => Err(format!("Design with id={} does not have a stored filepath.", design_id)),
...
mod tests {
	...
	async fn test_pool() -> SqlitePool {
		...
		sqlx::query("INSERT INTO hoops (name, max_width_mm, max_height_mm) VALUES ('Hoop A', 126, 126)")
			.execute(&pool)
			.await
	...
	async fn add_and_remove_project_membership_round_trip() {
		...
		let count_after_add =
			sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM project_designs WHERE design_id = 1 AND project_id = 1")
				.fetch_one(&pool)
				.await
