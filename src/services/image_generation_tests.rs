...
//
// This module was split out of image_generation.rs so the service file
// can stay focused on production logic. It is included via a #[path]
...
fn unsupported_extension_result_uses_unknown_when_no_extension() {
    ...
    assert!(
        ...
        "error should say unknown for no extension"
    );
}
...
fn three_d_style_high_contrast_all_variants() {
    ...
    assert_eq!(
        three_d_style_from_profile_name(Some("high_contrast")),
        expected
    );
    assert_eq!(
        three_d_style_from_profile_name(Some("highcontrast")),
        expected
    );
}
