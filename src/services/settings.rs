...
mod tests {
    ...
    fn description_for_key_known_keys() {
        ...
        assert!(description_for_key(KEY_IMPORT_COMMIT_BATCH_SIZE).contains("Maximum number of designs to persist"));
        assert!(description_for_key(KEY_IMPORT_LAST_BROWSE_FOLDER).contains("Most recently used folder"));
    ...
    async fn get_setting_with_default_returns_existing_value() {
        ...
        upsert_setting(&mut conn, KEY_AI_BATCH_SIZE, "42").await.unwrap();
    ...
    async fn upsert_setting_inserts_new_row() {
        let pool = setup_pool().await;
        let mut conn = pool.acquire().await.unwrap();
        upsert_setting(&mut conn, KEY_AI_DELAY, "3.5").await.unwrap();
    ...
    async fn upsert_setting_updates_existing_row() {
        ...
        let mut conn = pool.acquire().await.unwrap();
        upsert_setting(&mut conn, KEY_AI_DELAY, "3.5").await.unwrap();
        upsert_setting(&mut conn, KEY_AI_DELAY, "7.0").await.unwrap();
