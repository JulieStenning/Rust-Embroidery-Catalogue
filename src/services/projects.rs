...
mod tests {
    ...
    fn build_data_url_jpg_type_returns_jpeg_mime() {
        ...
        let expected = format!("data:image/jpeg;base64,{}", STANDARD.encode(vec![13, 14, 15]));
        assert_eq!(result, Some(expected));
    }
    ...
    fn build_data_url_unknown_image_type_defaults_to_png() {
        ...
        let expected = format!("data:image/png;base64,{}", STANDARD.encode(vec![16, 17, 18]));
        assert_eq!(result, Some(expected));
    }
    ...
    async fn get_projects_list_returns_empty_when_no_projects() {
        ...
        let result = get_projects_list(&state).await.expect("query should succeed");
    ...
    async fn get_projects_list_returns_single_project() {
        ...
        let projects = get_projects_list(&state).await.expect("query should succeed");
    ...
    async fn get_project_detail_includes_designs_with_images() {
        ...
        assert!(view.designs[0].image_data_url.as_deref().unwrap().starts_with("data:image/jpeg;base64,"));
        ...
        assert!(view.designs[1].image_data_url.as_deref().unwrap().starts_with("data:image/png;base64,"));
    }
    ...
    async fn delete_project_removes_project_successfully() {
        ...
        let result = delete_project(&state, 1).await.expect("delete should succeed");
    ...
    async fn delete_project_cascades_project_design_links() {
        ...
        delete_project(&state, 1).await.expect("delete should succeed");

        let links = sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*) FROM project_designs WHERE project_id = 1",
        )
        .fetch_one(&state.db)
        .await
        .expect("query should succeed");
    ...
    async fn get_project_print_view_returns_project_with_no_designs() {
        ...
        let view = get_project_print_view(&state, 1).await.expect("should succeed");
    ...
    async fn get_project_print_view_includes_designs_with_all_print_fields() {
        ...
        // X'AABB' + 'png' -> data URL
        assert!(design.image_data_url.as_deref().unwrap().starts_with("data:image/png;base64,"));
    }
