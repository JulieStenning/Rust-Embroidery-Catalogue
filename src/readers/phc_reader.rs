...
impl EmbroideryReader for PhcReader {
...
fn read_phc(data: &[u8]) -> Result<EmbPattern, Box<dyn std::error::Error>> {
    let mut pattern = EmbPattern::new();
    let mut cursor = Cursor::new(data);

    cursor.seek(SeekFrom::Start(0x4A))?;
    let pec_graphic_icon_height = read_u8(&mut cursor)? as u64;
    cursor.seek(SeekFrom::Current(1))?;
    let pec_graphic_byte_stride = read_u8(&mut cursor)? as u64;
    let color_count = read_u16_le(&mut cursor)? as usize;
    for _ in 0..color_count {
        let idx = match read_u8(&mut cursor) {
            Ok(v) => v,
    ...
    cursor.seek(SeekFrom::Start(0x2B))?;
    let pec_add = read_u8(&mut cursor)? as u64;
    cursor.seek(SeekFrom::Current(4))?;
    let pec_offset = read_u16_le(&mut cursor)? as u64;
    cursor.seek(SeekFrom::Start(pec_offset + pec_add))?;

    let bytes_in_section = read_u16_le(&mut cursor)? as i64;
    cursor.seek(SeekFrom::Current(bytes_in_section))?;
    let bytes_in_section2 = read_u32_le(&mut cursor)? as i64;
    cursor.seek(SeekFrom::Current(bytes_in_section2 + 10))?;
    let color_count2 = read_u8(&mut cursor)? as i64;
    cursor.seek(SeekFrom::Current(color_count2 + 0x1D))?;
...
mod tests {
    ...
    use std::fs;
    #[test]
    fn test_read_cake3_phc() {
        let path = "tests/testdata/Not Mandatory/Bean.phc";
        let data = fs::read(path).expect("Failed to read test PHC file");
        let pattern = PhcReader.read(&data).expect("Failed to parse PHC file");

        assert!(pattern.stitches.len() > 10, "expected parsed PHC stitches");
        assert!(
            pattern.stitches.iter().any(|s| s.x != 0.0 || s.y != 0.0),
            "expected non-zero PHC coordinates"
        );
        assert_eq!(
