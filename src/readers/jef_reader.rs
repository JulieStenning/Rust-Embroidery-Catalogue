...
// ---------------------------------------------------------------------------
// JEF header â€“ parsed with binrw.  Total size: 116 bytes before colour data.
// ---------------------------------------------------------------------------
...
/// Index 0 is a placeholder â€“ the Python reader treats colour index 0 as
/// "no thread", emitting a STOP instead of a COLOR_CHANGE.
fn get_jef_thread_set() -> Vec<EmbThread> {
    vec![
        // Index 0 â€“ placeholder (never exposed as a real thread)
...
pub fn read_jef(data: &[u8]) -> Result<EmbPattern, binrw::Error> {
    ...
    if count_colors > 1024 {
        // Sanity check â€“ no real JEF file has thousands of colours.
        return Err(binrw::Error::Io(std::io::Error::new(
    ...
    // --- 2. Read the colour-index table (count_colors Ã— i32 LE) -------------
    ...
    for &raw_idx in &raw_indices {
        ...
        if idx == 0 {
            // Colour index 0 â†’ push a dummy placeholder thread.  The stitch
    ...
    loop {
        ...
        match ctrl {
            0x02 => {
                // Jump â€“ use the absolute position encoded in extra bytes
            ...
            _ => {
                // Unknown control â€“ stop parsing (matching Python behaviour)
                break;
            }
        }
    }
