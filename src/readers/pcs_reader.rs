...
pub struct PcsReader;

impl EmbroideryReader for PcsReader {
    fn read(&self, data: &[u8]) -> Result<EmbPattern, Box<dyn std::error::Error>> {
        read_pcs(data)
    }
}

fn read_pcs(data: &[u8]) -> Result<EmbPattern, Box<dyn std::error::Error>> {
...
mod tests {
    ...
    use std::fs;
    #[test]
    fn test_read_cake3_pcs() {
        let path = "tests/testdata/Not Mandatory/Bean.pcs";
        let data = fs::read(path).expect("Failed to read test PCS file");
        let pattern = PcsReader.read(&data).expect("Failed to parse PCS file");

        assert!(pattern.stitches.len() > 10, "expected parsed PCS stitches");
        assert!(
            pattern.stitches.iter().any(|s| s.x != 0.0 || s.y != 0.0),
            "expected non-zero PCS coordinates"
        );
        assert_eq!(
