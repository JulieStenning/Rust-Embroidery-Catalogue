...
/// Absolute path to the repo's test-designs fixture directory. Derived from the
/// compile-time `CARGO_MANIFEST_DIR` (the folder holding `Cargo.toml`) so tests
/// resolve correctly regardless of which developer/drive the checkout lives on.
...
fn test_read_jef_synthetic_large_fixture() {
    ...
    let path = std::path::Path::new(FIXTURES_DIR).join("SyntheticLarge.jef");
