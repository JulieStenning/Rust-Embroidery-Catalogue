...
pub struct ZhsReader;

impl EmbroideryReader for ZhsReader {
    fn read(&self, data: &[u8]) -> Result<EmbPattern, Box<dyn std::error::Error>> {
        read_zhs(data)
    }
}

fn read_zhs(_data: &[u8]) -> Result<EmbPattern, Box<dyn std::error::Error>> {
    // Stub implementation
...
mod tests {
    ...
    use std::fs;
    #[test]
    fn test_read_cake3_zhs() {
        let path = "tests/testdata/Cake 3.zhs";
        let data = fs::read(path).expect("Failed to read test ZHS file");
        let pattern = ZhsReader.read(&data).expect("Failed to parse ZHS file");
