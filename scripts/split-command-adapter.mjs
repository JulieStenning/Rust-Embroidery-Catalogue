...
const fullOriginal = execSync(
  "git show HEAD:frontend/src/lib/api/commandAdapter.ts",
  { encoding: "utf8" },
);
...
function processSplitting(originalContent) {
  ...
  lines.forEach((line, i) => {
    ...
    if (match) {
      ...
      while (
        startLine > 0 &&
        (lines[startLine - 1].trim().startsWith("*") ||
          lines[startLine - 1].trim().startsWith("/**") ||
          lines[startLine - 1].trim().startsWith("//"))
      ) {
        startLine--;
...
export const MOCK_HOOPS = [
  ...
  const domainMap = {
    ...
    batchOperations: {
      ...
      funcs: [
        ...
        "browseTaggingFolder",
        "buildUnifiedBackfillWireRequest",
        "runUnifiedBackfill",
        "stopUnifiedBackfill",
  ...
  const originalFuncNames = funcPositions
    .map((f) => f.name)
    .filter((n) => n !== "invokeLoose");
  ...
  for (const [domainKey, domainDef] of Object.entries(domainMap)) {
    ...
    const usedTypes = allIpcTypes.filter((t) =>
      new RegExp(`\\b${t}\\b`).test(body),
    );
    const usedHelpers = ipcHelpers.filter((h) =>
      new RegExp(`\\b${h}\\b`).test(body),
    );
  ...
  console.log(
    "Updated",
    commandAdapterPath,
    "to re-export all domain adapters.",
  );
}
