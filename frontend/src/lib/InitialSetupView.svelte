<script>
  ...
  import { completeInitialSetup } from "./api/commandAdapter";

  /** Callback prop — called when the user has finished or skipped setup */
  let { onInitialSetupCompleted } = $props();

  /** 0 = Designers step, 1 = Sources step */
  let step = $state(0);
  let finishing = $state(false);
  let error = $state("");

  const designersCopy = {
    stepLabel: "Step 1 of 2 — Designers",
  ...
  let activeCopy = $derived(step === 0 ? designersCopy : sourcesCopy);
  let isLastStep = $derived(step === 1);
  ...
  /** Persist completion and advance to the main app. */
  async function finishSetup() {
    ...
    try {
      ...
      console.error("complete_initial_setup failed:", e);
    } finally {
      finishing = false;
    }
  }
...
<div class="max-w-5xl mx-auto py-6 px-4 space-y-4">
  ...
  <div class="bg-white rounded-xl shadow p-6 space-y-4">
    ...
    <p class="text-sm text-gray-600">
      ...
      easier to use. Both steps are optional and can be changed later via Settings.
    </p>
    ...
    <!-- Embedded admin view for the active step -->
    {#if step === 0}
