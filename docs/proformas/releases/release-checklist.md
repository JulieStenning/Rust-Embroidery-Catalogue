...
* [ ]  **Root `NOTICE` Attributions.** If non-dependency code or ported algorithms were added, add entries to **ACKNOWLEDGEMENTS & SPECIAL ATTRIBUTIONS** in the root `NOTICE` file, then run `node scripts/sync-licences.mjs` (or `npm run generate:licences`) so the `frontend/src/NOTICE` mirror used by the About pages stays in sync. See the pyembroidery entry for an example. **Never edit the root `LICENSE` file** — it must remain a byte-verbatim copy of the GNU General Public License v3.0 text with nothing prepended or appended; all attributions belong in `NOTICE`.
