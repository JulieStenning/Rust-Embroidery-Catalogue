...
When designs are imported or when you run **Step 1 (File & Folder Rules)** in **Admin → Batch Operations**:
...
Navigate to **Admin → Manage Data → Word Matches** (or `#/admin/data/tag-matches`).
...
When maintaining your general tag list in **Admin → Manage Data → Tags**:
- Every tag row has a dedicated **Matches** button. Clicking it opens a modal where you can view, add, or remove word matches for that specific tag.
